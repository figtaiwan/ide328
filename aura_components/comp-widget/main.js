...
var warning=[0,
	...
	'ERROR#03 : 禁止寫入系統區!',
	'ERROR#04 : 程式結構不對稱!']
var inputs=[], inp, tmp, cod, chr, ok, ready, error, i, iok, serialText=''
	...
	if (!active) {
        ...
        serialPort.on("data", function (data) { // 從 comp 到 console
        	...
        	if (chr!=='\r'&&cod<0x20) {
        		...
        		if (ok) {
	        		iok=serialText.length-ok.length
	        		ready=ok&&ok===serialText.substr(iok)
        		}
        		if (ready) {
        			serialText=serialText.substr(0,iok)+' <font color=green>好</font>'
